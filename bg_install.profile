<?php
// $Id: bg_install.profile,v 1.1.2.3.2.5 2011/01/06 23:25:36 goba Exp $
 ...
 */
function bg_install_form_install_configure_form_alter(&$form, $form_state) {
  ...
  $form['#submit'] = array('bg_install_custom_submit', 'install_configure_form_submit');
}

/**
 * Custom submit handler for the localized install.
 */
function bg_install_custom_submit($form, &$form_state) {
 ...
 */
function bg_install_install_tasks_alter(&$tasks, $install_state) {
 ...
 */
function bg_install_import_translation(&$install_state) {
